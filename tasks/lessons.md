...
## 2026-06-11 — 모델/요구사항 수정 후에는 이전 작업 흐름을 중단하고 기준을 다시 잡기

**상황**: 프로젝트 대청소 중 사용자가 "모델을 수정했습니다 기존에 했던걸 잠시 멈추고 다시 진행하세요"라고 정정했고,
이어서 "training session은 두고 진행"을 명시했다.

**처방**:
- 사용자가 모델·요구사항을 수정했다고 말하면, 이전 검증 결과나 삭제 후보를 그대로 이어 쓰지 말고 현재 워크트리/문서/TODO 기준을 다시 확인한다.
- 학습 세션·모델 산출물은 사용자 명시 보존 대상이면 캐시처럼 취급하지 않는다.
- 정리 작업은 재생성 가능한 캐시, 미구현 stub, legacy 문구, tracked runtime 오염부터 처리하고, 학습 산출물 삭제는 별도 확인 없이는 하지 않는다.

**적용 시점**: 대규모 cleanup/refactor 중 사용자가 "잠시 멈춰", "모델 바꿨어", "이건 두고 해" 같은 correction을 준 경우.
