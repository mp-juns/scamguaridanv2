...
> Ollama는 더 이상 필수가 아닙니다 (LLM이 Codex API로 교체됨).
...
| `ANTHROPIC_MODEL` | Codex 메인 분석 모델 | `Codex-sonnet-4-6` |
| `ANTHROPIC_HAIKU_MODEL` | 컨텍스트 챗봇 + 의도 분류 모델 | `Codex-haiku-4-5-20251001` |
...
| `ANTHROPIC_VISION_MODEL` | **v3** — vision OCR 모델 (이미지/PDF) | `Codex-sonnet-4-6` |
...
└──────────────────────────┬──────────────────────────────────────────┘
              ...
              (pipeline/runner.py)
                  ...
                  Phase 1: STT / OCR (stt.py + vision.py)  [v3 vision]
                           │  음성·영상 → Whisper, 이미지·PDF → Codex vision
                           │
...
Phase 1:   STT / OCR  — Whisper(음성) | Codex vision(이미지·PDF, v3)
...
1. **STT/OCR** (`stt.py` + `vision.py`): 텍스트 패스스루, YouTube URL → Whisper, 음성 파일 → Whisper, **이미지·PDF → Codex vision OCR + 시각 단서 통합** (v3). `stt.transcribe()` 가 확장자 보고 자동 라우팅.
...
| `vision.py` | **v3** Phase 1 — Codex vision OCR (이미지/PDF) | `transcribe(path)`, `transcribe_image()`, `transcribe_pdf()` → `VisionResult` |
...
| `llm_assessor.py` | Codex API 보조 판정 | `analyze_unified(text, scam_type, user_context)` → `UnifiedLLMResult` |
...
**TEXT 입력은 추가로 `context_chat.classify_intent()`로 의도 분류** (Codex Haiku, fast-path 우선):
...
사용자가 콘텐츠 보냄 (TEXT/URL/VIDEO/FILE)
   ...
   └─ 첫 질문 즉답 (Codex 호출 없는 static Q1, 응답 ~50ms)
         "📩 받았어요! 🔍 분석 시작했어요. 그 동안 정보 좀 여쭤볼게요"

사용자 답변 ↔ 봇이 본문 단서 짚어가며 능동 질문 (Q2부터 Codex Haiku, 1~3s)
   - context_chat.next_turn(input_type, history, transcript_text) 호출
   - Codex는 본문 + 누적 대화 모두 보고 다음 질문 생성
...
/admin/[runId] (에디터)
  ...
  ├─ POST /api/admin/runs/{id}/ai-draft   Codex API로 라벨링 초안 생성
...
| `pricing.py` | Codex/Whisper/Serper/VirusTotal 가격표 — `claude_cost(model, in, out)` 등 |
...
3. count >= 2 면 `classify_intent` (Codex Haiku) **호출 자체 skip** — 어뷰저 무료 LLM 통로 차단
...
이미지·PDF → 한국어 본문(텍스트 + 시각 단서). Codex vision (sonnet-4-6).
