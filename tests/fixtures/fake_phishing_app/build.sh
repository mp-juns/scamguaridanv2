#!/usr/bin/env bash
#
# 정적 분석 검증용 합성 피셔 APK 빌드. 무해(dead-code + RFC5737/.tk 비라우팅).
# dynamic_active_app/build.sh 와 동일 툴체인 — 실행 행동 대신 dead-code 신호만.
#
# 필요 도구: Android SDK (build-tools;34.0.0 + platforms;android-34) + JDK.
# 산출물: tests/fixtures/fake_phishing.apk
#
set -euo pipefail
...
# 4) zipalign + 자체 서명 (subject==issuer → apk_self_signed)
